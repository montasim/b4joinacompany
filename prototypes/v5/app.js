...
const legalContext = document.querySelector("[data-law-panel]")?.closest(".legal-context");
if (legalContext) {
  const tabs = [...legalContext.querySelectorAll("[data-law-topic]")];
  const panels = [...legalContext.querySelectorAll("[data-law-panel]")];

  const showLegalTopic = (topic, { moveFocus = false } = {}) => {
    tabs.forEach((tab) => {
      const active = tab.dataset.lawTopic === topic;
    ...
    panels.forEach((panel) => {
      panel.hidden = panel.dataset.lawPanel !== topic;
    });
  };

  tabs.forEach((tab, index) => {
    tab.addEventListener("click", () => showLegalTopic(tab.dataset.lawTopic));
    tab.addEventListener("keydown", (event) => {
      ...
      showLegalTopic(tabs[targetIndex].dataset.lawTopic, { moveFocus: true });
    });
  });
}
