...
export async function GET(request:Request){
  const actor=await workspaceActor(request); if(!actor)return apiError(401,"AUTH_REQUIRED","Sign in to access the private website workspace.");
  return NextResponse.json({items:await listCheckpoints(actor.userId)});
}
export async function POST(request:Request){
  const actor=await workspaceActor(request); if(!actor)return apiError(401,"AUTH_REQUIRED","Sign in to save a private website checkpoint.");
